...
class jsonApi {
    ...
    setConfig(apiConfig) {
        ...
        this.#privateData.responseHelper.baseUrl = this._apiConfig.pathPrefix
        this.#privateData.responseHelper.metadata = this._apiConfig.meta
    }
        ...
        this._resources[resourceConfig.resource] = {
            ...
            onCreate: Object.fromEntries(
                Object.entries(attributes).filter(
                    ([k, v]) => {
                        const helper = SchemaHelper.for(v)
                        return !helper.isReadOnly && !helper.isBelongsToRelationship
                    }
                )
            ),
            searchParams: {

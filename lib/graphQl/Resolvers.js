...
class Resolvers extends RetainsJsonApiPrivate {
    ...
    #generateFilterQueryFromAst(ast) {
         ...
         */
        const args = []
        for(const fieldAST of ast.fieldNodes.filter(fieldAST => fieldAST.arguments)) {
    ...
    #generateResourceFromArgs(args, resourceConfig) {
        ...
        return {
            ...
            attributes: Object.fromEntries(
                requestedEntries.filter(([attribute, joiSchema]) => !SchemaHelper.isRelationship(joiSchema)).map(
            ...
            relationships: Object.fromEntries(
                requestedEntries.filter(([attribute, joiSchema]) => SchemaHelper.isRelationship(joiSchema)).map(
                    ([attribute, joiSchema]) => {
                        const related = SchemaHelper.relationTypes(joiSchema)[0]
    ...
    search(resourceConfig, attribute, parent, args, req, ast) {
        ...
        } else if (!SchemaHelper.isRelationship(resourceConfig.attributes[attribute])) {

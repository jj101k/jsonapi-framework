...
class JoiConverter {
    ...
    #simpleAttribute(joiSchema) {
        if (SchemaHelper.type(joiSchema) == "array") {
            if (!this.#simpleAttribute(SchemaHelper.arrayItemType(joiSchema))) {
                throw new Error(`Unable to parse Joi type, got (array) ${SchemaHelper.debugString(joiSchema)}`)
            }

            return new graphQl.GraphQLList(graphQl.GraphQLString)
        } else if(SchemaHelper.type(joiSchema) == "any") {
            // This just returns the primitive type used here.
            const allowedValues = SchemaHelper.allowedValues(joiSchema)
            ...
            const type = JoiConverter.#joiSingleTypeToGraphQL[SchemaHelper.type(joiSchema)]
            if(!type) {
                throw new Error(`Unable to parse Joi type, got ${SchemaHelper.debugString(joiSchema)}`)
            }
            return type
        }
    }
    ...
    swap(joiSchema, graphQlResources) {
        ...
        if(SchemaHelper.isRelationship(joiSchema)) {
            const otherType = SchemaHelper.relationTypeCodes(joiSchema).join(this.#readTypes.UNION_JOIN_CONST)
            type = SchemaHelper.isToOneRelationship(joiSchema) ?

...
class responseHelper {
    ...
    async _enforceSchemaOnObject(item, schema) {
        ...
        try {
            const validationResult = await schema.validate(item)
    ...
    #generateDataItem(item, schema) {
        ...
        for(const someProperty of SchemaHelper.objectKeys(schema)) {
            if(SchemaHelper.isRelationship(schema.extract(someProperty))) {
    ...
    #generateRelationships(item, schema, linkProperties) {
        ...
        return Object.fromEntries(
            linkProperties.map(linkProperty => [
                linkProperty,
                this.#generateLink(item, schema.extract(linkProperty), linkProperty)
            ])
        )
    }
    ...
    #generateLink(item, schemaProperty, linkProperty) {
        ...
        if(SchemaHelper.isBelongsToRelationship(schemaProperty) && !item[linkProperty]) {
            // $FlowFixMe: the data property can be either undefined (not present), null or [ ]
            data = undefined
        } else if (SchemaHelper.isToOneRelationship(schemaProperty)) {
        ...
        } else if (SchemaHelper.isToManyRelationship(schemaProperty)) {
            // $FlowFixMe: the data property can be either undefined (not present), null or [ ]
        ...
        if (SchemaHelper.isBelongsToRelationship(schemaProperty)) {
            const as = SchemaHelper.inverseRelationshipName(schemaProperty)
            const relatedResource = SchemaHelper.relationTypes(schemaProperty)[0]

            meta = {
                relation: "foreign",
                belongsTo: relatedResource,
                as,
                many: SchemaHelper.isToManyRelationship(schemaProperty),
                readOnly: true
            }
            ...
            meta = {
                relation: "primary",
                // type: SchemaHelper.relationTypes(schemaProperty),
                readOnly: false
            }

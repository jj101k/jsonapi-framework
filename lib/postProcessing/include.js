...
class includeClass extends RetainsJsonApiPrivate {
    ...
    #buildURL(type, k, ids, includeTree) {
        const params = new URLSearchParams([
            ...
            ...Object.entries(includeTree._filter || {}),
        ].map(([k, v]) => [`filter[${k}]`, v]))
    ...
    #iterate(path, node, filter, validationErrors) {
        ...
        const resourceAttribute = node._resourceConfig.map(
        ...
        if (!node[first]) {
            node[first] = {
                _dataItems: [],
                _resourceConfig: relationInfo.map(a => this.jsonApi._resources[a]),
                _filter: {},
            }

            for (const i in restFilter) {
                if (Array.isArray(restFilter[i]) || typeof restFilter[i] === 'string') {
                    node[first]._filter[i] = restFilter[i]
                }
            }
        }
        this.#iterate(rest.join("."), node[first], restFilter)
    }
    /**
     *
     ...
     * @param {(errors: any | undefined, tree: import('../../types/postProcessing').includeTree) => any} callback
     * @returns
     */
    #arrayToTree(request, includes, filters, callback) {
        const validationErrors = [ ]
        const tree = {
            _dataItems: null,
            _resourceConfig: Array.isArray(request.resourceConfig) ?
    ...
    get action() {
        return (request, response, callback) => {
            ...
            if (!includes || !includes.length) return callback()
            ...
            this.#arrayToTree(request, includes, filters, (attErr, includeTree) => {
                ...
                includeTree._dataItems = Array.isArray(response.data) ?
                    response.data :
                    [ response.data ]
    ...
    #getDataItemsFromTree(tree) {
        ...
        for (const [i, v] of Object.entries(tree)) {
     ...
     * @param {*} callback
     */
    #fillIncludeTree(includeTree, request, callback) {
        ...
        const resourcesToFetch = [
            ...[...primaryRelations.entries()].map(
                ([relationKey, relation]) => ({
                    url: this.#buildURL(relation.type, "id", relation.ids, includeTree[relation.relation]),
                    as: relationKey
                })
            ),
            ...[...foreignRelations.entries()].map(
                ([relationKey, relation]) => ({
                    url: this.#buildURL(relation.belongsTo, relation.as, relation.ids, includeTree[relation.relation]),
                    as: relationKey
                })
            ),
        ]

...
class SchemaHelper {
    ...
    static arrayItemType(joiSchema) {
        return joiSchema._inner.items?.[0]
    }
    ...
    static debugString(joiSchema) {
        return JSON.stringify(joiSchema)
    }
    ...
    static isReadOnly(joiSchema) {
        return joiSchema._meta.includes("readonly")
    }
    ...
    static relationTypeCodes(joiSchema) {
        return joiSchema._settings?.__many ?? joiSchema._settings?.__one
    }

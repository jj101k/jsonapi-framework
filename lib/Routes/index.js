...
class Routes extends RetainsJsonApiPrivate {
    /**
     * @type {{new(privateData: jsonApiPrivate, helper: helper, jsonApi: jsonApi): import("./BaseError")}}
     */
    #errorClasses = []
    ...
    register() {
        ...
        for (const RouteClass of this.#routeClasses) {
            const router = new RouteClass(this.privateData, this.#helper, postProcess, this.jsonApi)
            router.bind()
        }
        for (const ErrorClass of this.#errorClasses) {
            const errorHandler = new ErrorClass(this.privateData, this.#helper, this.jsonApi)
            errorHandler.bind()
        }
    }
}

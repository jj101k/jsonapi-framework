...
class CreateRoute extends BaseRoute {
    ...
    async throwableHandler(request, resourceConfig, res) {
        ...
        await this.validate(newResource, resourceConfig.onCreate)
        ...
        const sanitisedData = await this.responseHelper._enforceSchemaOnObject(storedResource, OurJoi.object(resourceConfig.attributes))

        request.route.path += `/${storedResource.id}`
        res.set({
            Location: `${request.route.combined}/${storedResource.id}`,
        })
        return this.generateResponse(request, sanitisedData)
    }
}

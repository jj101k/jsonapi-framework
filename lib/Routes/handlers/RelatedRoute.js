...
class RelatedRoute extends BaseRoute {
    ...
    async throwableHandler(request, resourceConfig, res) {
        ...
        if (SchemaHelper.isBelongsToRelationship(relation)) {
            throw new JsonAPIError(
                "404",
                "EFOREIGN",
                "Relation is Foreign",
        ...
        request.resourceConfig = SchemaHelper.relationTypes(relation).map(
            resourceName => this.jsonApi._resources[resourceName]
        )
        if(SchemaHelper.isToOneRelationship(relation)) {

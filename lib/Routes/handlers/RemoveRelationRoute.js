...
class RemoveRelationRoute extends BaseRoute {
    ...
    async throwableHandler(request, resourceConfig, res) {
        ...
        const isMany = resourceConfig.attributes[relationName]._settings.__many
        const isOne = resourceConfig.attributes[relationName]._settings.__one
        ...
        await this.helper.validate(storedEntity, resourceConfig.onCreate)

...
class RemoveRelationRoute extends BaseRoute {
    ...
    async throwableHandler(request, resourceConfig, res) {
        ...
        const joiSchema = resourceConfig.attributes[relationName]
        let relationType
        if(SchemaHelper.isRelationship(resourceConfig.attributes[relationName])) {
            relationType = SchemaHelper.relationTypeCodes(resourceConfig.attributes[relationName])
        }

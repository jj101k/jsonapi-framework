...
class RemoveRelationRoute extends BaseRoute {
    ...
    /**
     *
     * @param {import("../../../types/Handler").JsonApiRequest<any, {type:
     * string, relation: string, data: {{type: string} | {type: string}[]}}>} request
    ...
    async throwableHandler(request, resourceConfig, res) {
        ...
        const relationType = helper.relationTypeCodes
        if(!relationType) {
            throw new Error(`${request.params.type}.${relationName} is not a relationship`)
        }

...
class filter {
    ...
    static #assertRelationshipIsNotForeign(resourceConfig, key) {
        if (SchemaHelper.isBelongsToRelationship(resourceConfig.attributes[key])) {
            throw new JsonAPIError(
                "403",
                "EFORBIDDEN",
                "Invalid filter",
     ...
     * @param {import("../types/ResourceConfig").Schema | null} attributeConfig
     * @throws
     */
    ...
    static #parseScalarFilterElementAssert(attributeConfig, scalarElement) {
        ...
        if (SchemaHelper.isRelationship(attributeConfig)) { // relationship attribute: no further validation
            return splitElement
        }

...
class SwaggerPaths {
    ...
    #actionPath(resourceName, actionName, action) {
        ...
        const handlerSchema = (methodName, description) => {
            if(SchemaHelper.for(action).actionFor(methodName)) {
    ...
    #deepPaths(resourceName, resourceConfig, relationName, relation) {
        const relationType = SchemaHelper.for(resourceConfig.attributes[relationName]).isToManyRelationship ? "many" : "one"
    ...
    #getActionObject(options) {
        ...
        if (options.parameters) {
            extraParameters = SchemaHelper.for(options.parameters).objectKeys.filter(
                (paramName) => paramName != "id" && paramName != "type"
            ).map((paramName) => {
                const helper = SchemaHelper.for(options.parameters.extract(paramName))
                return {
                    name: paramName,
                    in: "query",
                    description: helper.description || undefined,
                    required: helper.isRequired,
                    type: helper.type,
                }
            })
        } else {

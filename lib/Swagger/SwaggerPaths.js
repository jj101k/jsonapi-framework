...
class SwaggerPaths {
    ...
    #actionPath(resourceName, actionName, action) {
        ...
        const handlerSchema = (methodName, description) => {
            if(SchemaHelper.actionFor(action, methodName)) {
    ...
    #addData(body, data) {
        return {
            ...
            content: Object.fromEntries(
                Object.entries(body.content).map(([contentType, content]) => {
                    if(contentType == "application/json" || contentType == "application/vnd.api+json") {
                        return [contentType, {
                            ...content,
                            schema: ("properties" in content.schema) ?
                                {
                                    ...content.schema,
                                    properties: {
    ...
    #deepPaths(resourceName, resourceConfig, relationName, relation) {
        const relationType = SchemaHelper.isToManyRelationship(resourceConfig.attributes[relationName]) ? "many" : "one"
     ...
     * hasPathId: boolean, parameters: import("joi").Schema}} options
    ...
    #getActionObject(options) {
        ...
        if (options.parameters) {
            extraParameters = SchemaHelper.objectKeys(options.parameters).filter(
                (paramName) => paramName != "id" && paramName != "type"
            ).map((paramName) => {
                const schema = options.parameters.extract(paramName)
                return {
                    name: paramName,
                    in: "query",
                    description: SchemaHelper.description(schema) || undefined,
                    required: SchemaHelper.isRequired(schema),
                    type: SchemaHelper.type(schema),
                }
            })
        } else {
            extraParameters = []
    ...
    #getPathOperationObject(options) {
        ...
        /**
         * @type {import("openapi3-ts").RequestBodyObject}
         */
        let requestBody
        /**
         * @type {(import("openapi3-ts").ParameterObject | import("openapi3-ts").ReferenceObject)[]}
         */
        let handlerParams
        if (((options.handler === "search") || (options.handler === "find")) && !options.relation) {
            handlerParams = [
    ...
    #pathDefinition(resourceConfig) {
        ...
        for(const [attributeName, attributeConfig] of Object.entries(resourceConfig.attributes)) {
            if(!SchemaHelper.isRelationship(attributeConfig) || SchemaHelper.isBelongsToRelationship(attributeConfig)) continue
            const relationEntityName = SchemaHelper.relationTypes(attributeConfig)[0]

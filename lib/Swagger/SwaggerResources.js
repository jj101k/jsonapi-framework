...
class SwaggerResources {
    ...
    #attributeSchema(joiSchema) {
        const type = SchemaHelper.type(joiSchema)
        const description = SchemaHelper.description(joiSchema) || undefined
        /**
         * @type {JsonSchema}
         */
        let schema
        if (type === "date") {
            ...
            let properties
            const itemType = SchemaHelper.arrayItemType(joiSchema)
            if (itemType && SchemaHelper.type(itemType) == "object") {
                properties = Object.fromEntries(
                    SchemaHelper.objectKeys(itemType).map((k) => [k, {type: SchemaHelper.type(itemType.extract(k))}])
                )
            } else {

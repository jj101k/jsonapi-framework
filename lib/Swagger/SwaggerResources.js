...
class SwaggerResources {
    ...
    #attributeSchema(joiSchema) {
        ...
        if (type === "date") {
            ...
            let properties
            const itemType = helper.arrayItemType
            if (itemType) {
                const itemTypeHelper = SchemaHelper.for(itemType)
                if (itemTypeHelper.type == "object") {
                    properties = Object.fromEntries(
                        itemTypeHelper.objectKeys.map((k) => [k, {type: SchemaHelper.for(itemType.extract(k)).type}])
                    )
                } else {
    ...
    #resourceDefinition(resourceConfig) {
        ...
        for (const [attribute, joiSchema] of Object.entries(resourceConfig.attributes)) {
            ...
            if(!SchemaHelper.for(joiSchema).isRelationship) {

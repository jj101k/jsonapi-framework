...
class findRoute extends BaseRoute {
    ...
    async handler(request, resourceConfig, res) {
        try {
            ...
            await this.postProcess.fetchForeignKeys(request, resource, resourceConfig.attributes)
            ...
            response = this.responseHelper._generateResponse(request, resourceConfig, sanitisedData)
            response.included = [ ]

...
foreignKeySearchRoute.register = (privateData, helper, postProcess) => {
    privateData.router.bindRoute({
        ...
        async.waterfall([
            ...
            callback => {
                response = privateData.responseHelper._generateResponse(request, resourceConfig, searchResults)
                response.included = [ ]

...
class ChainHandler extends Handler {
     ...
     *
     * @param {(callback: (err, ...results) => any) => any} f
     * @returns
     */
    ...
    static chainObjectToPromise(o, ...methods) {
         ...
         */
        const methodsAny = methods
        return new Proxy(
            o,
            {
                get(o, n) {
                    if(methodsAny.includes(n)) {
    ...
    async #asyncAction(argsIn, action, request, callback) {
        ...
        try {
            ...
            if(otherHandler && typeof otherHandler[lowerAction] === "function") {
                args = await ChainHandler.#chainToPromise(cb => otherHandler[lowerAction](...args, cb))
            }
    ...
    #buildAction(action) {
        ...
        return (request, ...args) => {
            ...
            if(argsIn[argsIn.length - 1] instanceof Function) {
                ...
                if (this.otherHandler && typeof this.otherHandler[lowerAction] === "function") {

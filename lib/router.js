...
/**
 * @typedef {import("./RerouteRequest")} RerouteRequest
 * @typedef {import("../types/Handler").JsonApiRequestBasic} JsonApiRequestBasic
...
class router extends RetainsJsonApiPrivate {
     ...
     * @param {import("express").Response | import("./rerouter").rerouteResponse} res
     * @param {() => any} callback
     * @returns
     */
    ...
    /**
     *
     * @param {import("express").Request | RerouteRequest} req
     * @returns {JsonApiRequestBasic} A more structured form of the original request
     */
    #getParams(req) {
    ...
    /**
     * @type {{[verb: string]: {[path: string]: (req: RerouteRequest |
     * import("express").Request, res: import("./rerouter").rerouteResponse |
     * import("express").Response, modifiedRequest:
     * Partial<JsonApiRequest>) => void}}}}
     */
    _routes = { }
    ...
    applyMiddleware() {
        this.expressServer.use((req, res, next) => {
            ...
            res.locals.outputTypesToUse = outputTypesToUse
        ...
        this.expressServer.use((req, res, next) => {
            res.set({
                "Content-Type": res.locals.outputTypesToUse[0],
     ...
     * @param {(req: JsonApiRequest, rc:
     * import("../types/jsonApi").ResourceConfig, res:
     * import("express").Response | import("./rerouter").rerouteResponse) => any} callback
     */
    bindRoute(config, callback) {
        ...
        this._routes[verb][config.path] = (req, res, extras) => {
            ...
            res.locals._request = request
            res.locals._startDate = new Date()
    ...
    /**
     *
     * @param {(request: JsonApiRequestBasic, res: import("express").Response) => any} callback
     */
    bind404(callback) {
    ...
    /**
     *
     * @param {(request: JsonApiRequestBasic, res: import("express").Response, error: any, next: Function) => any} callback
     */
    ...
    /**
     *
     * @param {import("express").Response & {locals: {_request:
     * JsonApiRequest, _startDate: Date | null}}} res
     * @param {*} payload
     * @param {number} httpCode
     */
    sendResponse(res, payload, httpCode) {
        if(res.locals._startDate) {
            const timeDiff = new Date().valueOf() - res.locals._startDate?.valueOf()
            metrics.processResponse(res.locals._request, httpCode, payload, timeDiff)
        } else {
            metrics.processResponse(res.locals._request, httpCode, payload, -1)
        }

'use strict'

const debug = require('./debugging')

/**
 *
 */
class handlerEnforcer {
    /**
     *
     * @param {*} handlers
     * @returns
     */
    static wrapped(handlers) {
        return {
            ...handlers,
            create: handlerEnforcer._create(handlers),
            delete: handlerEnforcer._delete(handlers),
            find: handlerEnforcer._find(handlers),
            search: handlerEnforcer._search(handlers),
            update: handlerEnforcer._update(handlers),
        }
    }

    /**
     *
     * @param {*} handlers
     * @param {*} operation
     * @param {*} outCount
     * @returns
     */
    static _wrapHandler(handlers, operation, outCount) {
        if (typeof outCount !== 'number') {
            throw new Error('Invalid use of handlerEnforcer._wrapHandler!')
        }

        const original = handlers[operation]
        if (!original) return null
        return function () {
            const argsIn = Array.prototype.slice.call(arguments)
            const requestParams = argsIn[0].params
            const callback = argsIn.pop()
            argsIn.push(function () {
                let argsOut = Array.prototype.slice.call(arguments)
                argsOut = argsOut.slice(0, outCount)
                // $FlowFixMe: We've already ruled out any other possible types for outCount?
                while (argsOut.length < outCount) {
                    argsOut.push(null)
                }
                debug.handler[operation](JSON.stringify(requestParams), JSON.stringify(argsOut))
                return callback.apply(null, argsOut)
            })
            original.apply(handlers, argsIn)
        }
    }

    /**
     *
     * @param {*} handlers
     * @returns
     */
    static _search = handlers => handlerEnforcer._wrapHandler(handlers, 'search', 3)

    /**
     *
     * @param {*} handlers
     * @returns
     */
    static _find = handlers => handlerEnforcer._wrapHandler(handlers, 'find', 2)

    /**
     *
     * @param {*} handlers
     * @returns
     */
    static _create = handlers => handlerEnforcer._wrapHandler(handlers, 'create', 2)

    /**
     *
     * @param {*} handlers
     * @returns
     */
    static _update = handlers => handlerEnforcer._wrapHandler(handlers, 'update', 2)

    /**
     *
     * @param {*} handlers
     * @returns
     */
    static _delete = handlers => handlerEnforcer._wrapHandler(handlers, 'delete', 1)
}


module.exports = handlerEnforcer
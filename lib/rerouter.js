...
class rerouter extends RetainsJsonApiPrivate {
    ...
    async route(newRequest) {
        ...
        const req = {
            ...
            params: this.#mergeParams(url.parse(newRequest.uri.split('?')[1] || { }), newRequest.params)
        }
    ...
    #generateSanePath(newRequest) {
         ...
         * @type {string}
         */
        let path = newRequest.uri.replace(new RegExp('^https?://[^/]*'), "")
    ...
    /**
     *
     * @param {{[key: string]: any}} objA
     * @param {{[key: string]: any}} objB
     * @returns
     */
